...
export function resolveFirstNewEnrolleeDisplayDate(
  ...
  const joinedAtWeekday = getJstDayOfWeek(joinedAtDay);
  const targetWeekday = getDayOfWeekIndex(targetDayOfWeek);
  const dayOffset = (targetWeekday - joinedAtWeekday + 7) % 7;
